...
namespace EMS.ViewModels.Models
{
    ...
    {
        static public EmployeeModel CreateEmpty() => new EmployeeModel { EmployeeID = -1, IsEmpty = true };
        ...
        {
            if (source is EmployeeModel model)
            {
                Merge(model);
            }
        }

        public void Merge(EmployeeModel source)
        {
            if (source != null)
            {
